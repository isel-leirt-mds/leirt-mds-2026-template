package pt.isel.mpd.utils.resources;
...
public class ResourceUtils {
    ...
    private static String convert(String path) {
        ...
        else if (path.startsWith("http://")) start = 7;
        var end = path.lastIndexOf('&');
        if (end == -1) end = path.lastIndexOf('?');
        if (end == -1) end = path.length();
        var result =   path.substring(start, end)
            .replace('&', '-')
            .replace('/', '.')
            .replace( '?','-')
            .replace( ',','-');
    ...
    public static BufferedReader openResource(String path) {
        try {
            ...
            return new BufferedReader(new InputStreamReader(keyFile.openStream()));
